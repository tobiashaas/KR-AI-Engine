#!/usr/bin/env python3
"""
🚀 Improved Hybrid Document Classifier
Enhanced version with corrected error code extraction and series detection

Features:
- Fixed error code extraction patterns
- Series detection for better filtering
- Improved model number validation
- Better part number extraction
- Enhanced manufacturer-specific patterns
"""
...
class ImprovedHybridClassifier:
    """Improved hybrid document classifier with better error code and series detection"""
    
    def __init__(self):
        self.manufacturer_patterns = self._load_manufacturer_patterns()
        self.document_type_patterns = self._load_document_type_patterns()
        self.error_code_patterns = self._load_error_code_patterns()
        self.series_patterns = self._load_series_patterns()
    ...
    def _load_manufacturer_patterns(self) -> Dict:
        """Load comprehensive manufacturer detection patterns with series info"""
        return {
            'hp': {
                ...
                'series': {
                    'LaserJet Pro': [r'E\d+', r'X\d+', r'M\d+', r'P\d+'],
                    'LaserJet Enterprise': [r'E\d+', r'X\d+'],
                    'DeskJet': [r'\d+', r'Plus\s+\d+', r'Ink\s+Advantage'],
                    'OfficeJet': [r'Pro\s+\d+', r'\d+', r'Enterprise'],
                    'Color LaserJet': [r'Pro\s+\d+', r'Enterprise\s+\d+']
                },
                'model_series': [
                    r'LaserJet\s+Pro\s+([A-Z0-9]+)', r'LaserJet\s+Enterprise\s+([A-Z0-9]+)',
                    r'DeskJet\s+([A-Z0-9]+)', r'OfficeJet\s+Pro\s+([A-Z0-9]+)',
                    r'Color\s+LaserJet\s+Pro\s+([A-Z0-9]+)', r'E\s*(\d+)', r'X\s*(\d+)',
                    r'M\s*(\d+)', r'P\s*(\d+)'
                ],
            ...
            'konica_minolta': {
                ...
                'series': {
                    'BizHub': [r'C\d+[a-z]?', r'P\d+[a-z]?', r'\d+[a-z]?'],
                    'AccurioPress': [r'C\d+', r'\d+'],
                    'MultiX': [r'\d+[a-z]?']
                },
                'model_series': [
    ...
    def _load_error_code_patterns(self) -> Dict:
        """Load manufacturer-specific error code patterns"""
        return {
            'hp': {
                'patterns': [
                    r'error\s+code\s+(\d{2}\.\d{2}\.\d{2})',  # HP format: XX.XX.XX
                    r'error\s+(\d{2}\.\d{2}\.\d{2})',         # XX.XX.XX
                    r'code\s+(\d{2}\.\d{2}\.\d{2})',          # XX.XX.XX
                    r'(\d{2}\.\d{2}\.\d{2})\s+error',         # XX.XX.XX error
                    r'error\s+(\d{2}\.\d{2})',                # XX.XX
                    r'(\d{2}\.\d{2})\s+error',                # XX.XX error
                ],
                'validation': r'^\d{2}\.\d{2}(?:\.\d{2})?$'  # Must match HP format
            },
            'konica_minolta': {
                'patterns': [
                    r'error\s+code\s+(\d+\.\d+\.\d+)',        # KM format: X.X.X
                    r'error\s+(\d+\.\d+\.\d+)',               # X.X.X
                    r'code\s+(\d+\.\d+\.\d+)',                # X.X.X
                    r'(\d+\.\d+\.\d+)\s+error',               # X.X.X error
                    r'error\s+(\d+\.\d+)',                    # X.X
                    r'(\d+\.\d+)\s+error',                    # X.X error
                ],
                'validation': r'^\d+\.\d+(?:\.\d+)?$'         # Must match KM format
            },
            'lexmark': {
                'patterns': [
                    r'error\s+code\s+(\d{3,4})',              # Lexmark format: XXX or XXXX
                    r'error\s+(\d{3,4})',                     # XXX or XXXX
                    r'code\s+(\d{3,4})',                      # XXX or XXXX
                    r'(\d{3,4})\s+error',                     # XXX or XXXX error
                ],
                'validation': r'^\d{3,4}$'                    # Must be 3-4 digits
            },
            'utax': {
                'patterns': [
                    r'error\s+code\s+(\d{5})',                # UTAX format: XXXXX
                    r'error\s+(\d{5})',                       # XXXXX
                    r'code\s+(\d{5})',                        # XXXXX
                    r'(\d{5})\s+error',                       # XXXXX error
                ],
                'validation': r'^\d{5}$'                      # Must be 5 digits
            }
        }
    
    def _load_series_patterns(self) -> Dict:
        """Load series detection patterns for better filtering"""
        return {
            'hp': {
                'LaserJet Pro': {
                    'keywords': ['laserjet pro', 'pro series'],
                    'models': [r'E\d+', r'X\d+', r'M\d+', r'P\d+'],
                    'description': 'Professional laser printer series'
                },
                'LaserJet Enterprise': {
                    'keywords': ['laserjet enterprise', 'enterprise series'],
                    'models': [r'E\d+', r'X\d+'],
                    'description': 'Enterprise laser printer series'
                },
                'DeskJet': {
                    'keywords': ['deskjet', 'inkjet'],
                    'models': [r'\d+', r'Plus\s+\d+'],
                    'description': 'Consumer inkjet printer series'
                },
                'OfficeJet': {
                    'keywords': ['officejet', 'office jet'],
                    'models': [r'Pro\s+\d+', r'\d+'],
                    'description': 'Office multifunction printer series'
                }
            },
            'konica_minolta': {
                'BizHub': {
                    'keywords': ['bizhub', 'biz hub'],
                    'models': [r'C\d+[a-z]?', r'P\d+[a-z]?'],
                    'description': 'Business multifunction printer series'
                },
                'AccurioPress': {
                    'keywords': ['accuriopress', 'press series'],
                    'models': [r'C\d+'],
                    'description': 'Production printing press series'
                }
            },
            'lexmark': {
                'MS Series': {
                    'keywords': ['ms series', 'ms\d+'],
                    'models': [r'MS\d+'],
                    'description': 'Monochrome laser printer series'
                },
                'MX Series': {
                    'keywords': ['mx series', 'mx\d+'],
                    'models': [r'MX\d+'],
                    'description': 'Multifunction printer series'
                },
                'CX Series': {
                    'keywords': ['cx series', 'cx\d+'],
                    'models': [r'CX\d+'],
                    'description': 'Color multifunction printer series'
                }
            },
            'utax': {
                'P-C Series': {
                    'keywords': ['p-c series', 'p-c\d+'],
                    'models': [r'P-C\d+[A-Z]+'],
                    'description': 'Professional color printer series'
                },
                'Ci Series': {
                    'keywords': ['ci series', '\d+ci'],
                    'models': [r'\d+ci'],
                    'description': 'Color imaging printer series'
                }
            }
        }
    ...
    def classify_document(self, filename: str, content: str = "") -> Dict:
        """Improved hybrid document classification with better error codes and series detection"""
        logger.info(f"🚀 Starting improved hybrid classification for: {filename}")
        ...
        # 4. Extract technical information with improved patterns
        if content.strip():
            tech_info = self._extract_technical_information_improved(content, final_classification['manufacturer'])
        ...
        # 6. Calculate hybrid confidence
        result['analysis']['hybrid_confidence'] = self._calculate_hybrid_confidence(
            filename_result, content_result, final_classification
        )
        
        logger.info(f"✅ Improved hybrid classification completed: {result['classification']['document_type']} "
    ...
    def _extract_technical_information_improved(self, content: str, manufacturer: str) -> Dict:
        """Extract technical information with improved error code patterns"""
        return {
            'model_numbers': self._extract_models_from_content(content, manufacturer),
            'error_codes': self._extract_error_codes_improved(content, manufacturer),
            'part_numbers': self._extract_part_numbers_improved(content),
            'technical_info': self._analyze_technical_content(content)
        }
    
    def _extract_error_codes_improved(self, content: str, manufacturer: str) -> List[str]:
        """Extract error codes with manufacturer-specific validation"""
        error_codes = []
        
        # Get manufacturer-specific patterns
        manufacturer_key = manufacturer.lower()
        if manufacturer_key in self.error_code_patterns:
            patterns = self.error_code_patterns[manufacturer_key]['patterns']
            validation_pattern = self.error_code_patterns[manufacturer_key]['validation']
    ...
    def _extract_part_numbers_improved(self, content: str) -> List[str]:
        """Extract part numbers with better validation"""
        part_patterns = [
            r'part\s+number[:\s]+([A-Z0-9\-]{6,12})',  # Part Number: XXXXXX
            r'pn[:\s]+([A-Z0-9\-]{6,12})',             # PN: XXXXXX
            r'([A-Z]{2,4}\d{4,8}[A-Z]?)',              # Standard part format
            r'([A-Z]\d{6,10})',                        # Single letter + digits
        ]
        ...
        for pattern in part_patterns:
            matches = re.findall(pattern, content, re.IGNORECASE)
            for match in matches:
                # Basic validation: 6-12 characters, alphanumeric with dashes
                if 6 <= len(match) <= 12 and re.match(r'^[A-Z0-9\-]+$', match.upper()):
                    part_numbers.append(match.upper())
    ...
    def _detect_series(self, filename: str, content: str, manufacturer: str) -> Dict:
        ...
        manufacturer_key = manufacturer.lower()
        if manufacturer_key not in self.series_patterns:
            return series_info
        ...
        for series_name, series_data in self.series_patterns[manufacturer_key].items():
            score = 0.0
            
            # Check keywords in content
            for keyword in series_data['keywords']:
            ...
            for model_pattern in series_data['models']:
                content_matches = len(re.findall(model_pattern, content, re.IGNORECASE))
                filename_matches = len(re.findall(model_pattern, filename, re.IGNORECASE))
                score += content_matches * 0.4
                score += filename_matches * 0.2
            
            series_scores[series_name] = score
        
        # Find best matching series
        if series_scores:
            best_series = max(series_scores, key=series_scores.get)
            if series_scores[best_series] > 0:
                series_info['detected_series'] = best_series
                series_info['confidence'] = min(series_scores[best_series] / 3.0, 1.0)
                series_info['description'] = self.series_patterns[manufacturer_key][best_series]['description']
                
                # Extract models for this series
                series_models = []
                for model_pattern in self.series_patterns[manufacturer_key][best_series]['models']:
                    matches = re.findall(model_pattern, content, re.IGNORECASE)
                    series_models.extend(matches)
                series_info['series_models'] = list(set(series_models))
...
def main():
    """Test the improved hybrid document classifier"""
    print("🚀 Improved Hybrid Document Classifier")
    print("=" * 50)
